...
export function validateSsh(
  ...
  if (parsed.knockd) {
    ...
    if (k.enabled === true) {
      ...
      if (
        k.sequence &&
        k.sequence.length === DEFAULT_KNOCKD_SEQUENCE.length &&
        k.sequence.every((p, i) => p === DEFAULT_KNOCKD_SEQUENCE[i])
      ) {
        addError(r, file,
          "knockd.sequence is still the default sequence generated by 'soverstack init'. This sequence is public and must be changed for security",
          "knockd.sequence",
          "Replace with your own unique port sequence (e.g. [12345, 54321, 33333])",
        );
      }
    }
  }
